...
class NhomModel extends DB
{
    ...
    public function getAllGroup($manhom){
        // $sql = "SELECT * FROM nhom WHERE manhom = '$manhom'";
        // $result = mysqli_query($this->con,$sql);
        // $row = mysqli_fetch_assoc($result);
        // $mamon = $row['mamonhoc'];
        // $sql = "SELECT * FROM nhom WHERE mamonhoc = '$mamon' AND manhom != '$manhom'";
        $id = $_SESSION['user_id'];
        $sql_nguoidung = "SELECT * FROM nguoidung where id = '$id'";
        $ng = mysqli_query($this->con,$sql_nguoidung);
        $ng = mysqli_fetch_assoc($ng);
        $malop = $ng['malop'];
        $sql_lop = "SELECT * FROM lop WHERE malop = $malop";
        $ng = mysqli_query($this->con,$sql_lop);
        $ng = mysqli_fetch_assoc($ng);
        $khoa = $ng['makhoa'];
        $sql = "SELECT * FROM lop WHERE trangthai = '1' AND makhoa = '$khoa'";
        $result = mysqli_query($this->con,$sql);
        $rows = array();
        while($row = mysqli_fetch_assoc($result)){
            $rows[]=$row;
    ...
    public function addUserAllGroup($nhomhientai,$manhom){
        $sql = "SELECT * FROM nguoidung WHERE malop = $manhom";
        $result = mysqli_query($this->con,$sql);
        $val = true;
        while($row = mysqli_fetch_assoc($result)){
            $check = $this->join($nhomhientai,$row['id']);
            if(!$check) $val = false;
        }
        return $val;
    }
