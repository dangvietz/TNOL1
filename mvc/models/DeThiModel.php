...
class DeThiModel extends DB
{
        ...
        if (isset($args["custom"]["function"])) {
            ...
            switch ($func) {
                ...
                case "getAllCreatedTest":
                    ...
                    $query = "SELECT * FROM dethi DT, monhoc MH, giaodethi GDT, nhom N , nguoidung ND WHERE DT.nguoitao=ND.id AND DT.monthi = MH.mamonhoc AND DT.made = GDT.made AND N.manhom = GDT.manhom AND nguoitao = '" . $args['id'] . "' AND DT.trangthai = 1";
                    if (isset($filter)) {
                        switch ($filter) {
                            case "0";
                                $query .= " AND CURRENT_TIMESTAMP() < thoigianbatdau";
    ...
    public function getPoetryAll($query,$filter)
    {
    $sql = "SELECT * FROM cathi, nguoidung, dethi WHERE cathi.magv = nguoidung.id AND cathi.made = dethi.made AND cathi.tencathi LIKE '%$query%'";
    if (isset($filter)) {
        switch ($filter) {
            case "0";
                $sql .= " AND CURRENT_TIMESTAMP() < dethi.thoigianbatdau";
                break;
            case "1";
                $sql .= " AND CURRENT_TIMESTAMP() BETWEEN dethi.thoigianbatdau AND dethi.thoigianketthuc";
                break;
            case "2";
                $sql .= " AND CURRENT_TIMESTAMP() > dethi.thoigianketthuc";
                break;
            default:
        }
    }
    $sql = $sql." ORDER BY cathi.macathi DESC";
    $result = mysqli_query($this->con, $sql);
    $rows = array();
    while ($row = mysqli_fetch_assoc($result)) {
        $rows[] = $row;
    ...
    public function getPoetryDetail($id)
    {
        $sql = "SELECT * FROM cathi, dethi where macathi = $id AND cathi.made=dethi.made";
        $result = mysqli_query($this->con, $sql);
    ...
    public function createPoetry($tencathi, $dethi, $cathi, $ngaythi, $loaide,$thoigian)
    {
        $magv = $_SESSION['user_id'];
        $sql = "INSERT INTO `cathi`(`ngaythi`, `tencathi`, `khunggio`, `loaikythi`, `made`, `magv`) VALUES ('$ngaythi','$tencathi','$cathi','$loaide','$dethi','$magv')";
        $check = true;
        $result = mysqli_query($this->con, $sql);
        if (!$result) {
            $check = false;
        }
        $thoigianArray = explode('-', $cathi);
        $gioBatDau = trim($thoigianArray[0]);
        $gioKetThuc = trim($thoigianArray[1]);

        // Chuyển đổi thành timestamp
        $thoigianbatdau_timestamp = strtotime($ngaythi . ' ' . $gioBatDau);
        $thoigianketthuc_timestamp = strtotime($ngaythi . ' ' . $gioKetThuc);
    ...
    {
        $sql = "UPDATE `dethi` SET `thoigianbatdau`='$thoigianbatdau', `thoigianketthuc`='$thoigianketthuc', `thoigianthi`='$thoigian' WHERE made = $dethi";
        $check = true;
        $result = mysqli_query($this->con, $sql);
        if (!$result) {
            $check = false;
    ...
    public function resetThoiGian($dethi)
    {
        $sql = "UPDATE `dethi` SET `thoigianbatdau`= NULL , `thoigianketthuc`= NULL, `thoigianthi`='0' WHERE made = $dethi";
        $result = mysqli_query($this->con, $sql);
    }

    public function deletePoetry($macathi)
    {
        $sql_get = "SELECT * FROM `cathi` where macathi = $macathi";
        $ct = mysqli_query($this->con, $sql_get);
        $row = mysqli_fetch_assoc($ct);
        $made = $row['made'];
        $this->resetThoiGian($made);
        $sql = "DELETE FROM `cathi` WHERE macathi = $macathi";
        $result = mysqli_query($this->con, $sql);
        $check = true;
        $result = mysqli_query($this->con, $sql);
        if (!$result) {
    ...
    public function updatePoetry($macathi, $tencathi, $dethi, $cathi, $ngaythi, $loaide, $thoigian)
    {
        $gv = $_SESSION['user_id'];
        $sql = "UPDATE `cathi` SET `ngaythi`='$ngaythi',`tencathi`='$tencathi',`khunggio`='$cathi',`loaikythi`='$loaide',`made`='$dethi',`magv`='$gv' WHERE `macathi`='$macathi'";
        $check = true;
        $result = mysqli_query($this->con, $sql);
        if (!$result) {
            $check = false;
